...
// --- 6. ARAMA MANTIĞI (DÜZENLENEN KISIM) ---
function setupSearch() {
    ...
    input?.addEventListener('input', function () {
        ...
        // Genişletilmiş filtreleme: Sözcük, Eş Anlamlılar veya Bilimsel Ad
...
function displaySuggestions(matches, q) {
    ...
    matches.slice(0, 10).forEach(m => {
        ...
        // Ekranda ne görüneceğine dair mantık:
        // Eğer arama terimi eş anlamlıda veya bilimseldedeyse, o kelimeyi de göster
        let displayText = m.Sözcük;
        ...
        if (!sozcuk.startsWith(q)) {
             if (esAnlam.includes(q)) {
                 // Eşleşen eş anlamlıyı bulup yanına ekliyoruz
                 const foundSyn = m['Eş Anlamlılar'].split(',').find(s => normalizeString(s).includes(q));
                 if(foundSyn) displayText = `${foundSyn.trim()} → ${m.Sözcük}`;
             } else if (bilimsel.includes(q)) {
                 displayText = `${m.Bilimsel} → ${m.Sözcük}`;
             }
        }

        d.innerHTML = `<span class="font-bold">${isGreek ? convertToGreek(displayText) : displayText}</span>`;
        d.onclick = () => selectWord(m, m.Sözcük); // Her zaman ana Sözcük'e gider
        div.appendChild(d);
    });
