...
let sortConfig = { key: 'harf', direction: 'asc' }; // Sıralama takibi
...
function renderAlphabetStats() {
    ...
    // Arayüzü Oluştur
    container.innerHTML = `
        <div class="col-span-full mb-4 flex justify-center gap-2">
            ${['harf', 'başta', 'toplam'].map(key => `
                <button onclick="toggleSort('${key}')" class="px-4 py-2 rounded-lg bg-subtle-light dark:bg-subtle-dark border border-subtle-light dark:border-subtle-dark font-bold text-xs uppercase transition-all hover:border-primary select-none">
                    ${isGreek ? convertToGreek(key) : key} ${sortConfig.key === key ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
                </button>
            `).join('')}
        </div>
    `;
