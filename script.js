...
function setupSearch() {
    ...
    input?.addEventListener('input', function () {
        ...
        allWords.forEach(row => {
            const sozcuk = (row.Sözcük || "").toLocaleLowerCase('tr-TR');
            const bilimsel = (row.Bilimsel || "").toLocaleLowerCase('tr-TR');
            const esAnlamStr = (row['Eş Anlamlılar'] || "").toLocaleLowerCase('tr-TR');
            const synArray = esAnlamStr.split(',').map(s => s.trim());

            if (sozcuk.startsWith(q)) matches.push({ data: row, text: row.Sözcük, subText: null });
            if (bilimsel.startsWith(q)) matches.push({ data: row, text: row.Bilimsel, subText: row.Sözcük });
            synArray.forEach(s => {
                if (s.startsWith(q)) matches.push({ data: row, text: s, subText: row.Sözcük });
            });
        });
...
function displaySuggestions(matches) {
    ...
    matches.filter(m => { if (seen.has(m.text)) return false; seen.add(m.text); return true; }).slice(0, 15).forEach(m => {
        ...
        d.innerHTML = `<span class="font-bold">${isGreek ? convertToGreek(m.text) : m.text}</span>
...
function renderHistory() {
    ...
    searchHistory.forEach(item => {
        ...
        d.innerHTML = `<span class="font-bold opacity-70">${display}</span>${item.subText ? `<span class="opacity-30 ml-2 text-sm">${subDisplay}</span>` : ''}`;
...
async function submitFeedback() {
    ...
    try {
        const scriptURL = 'GOOGLE_APPS_SCRIPT_URL_BURAYA';
        const formData = new FormData();
        formData.append('İletişim', contact.value);
        formData.append('Mesaj', message.value);

        // await fetch(scriptURL, { method: 'POST', body: formData }); // Aktif etmek için başındaki yorumu kaldırın
        ...
        btn.disabled = false; btn.innerText = "Gönder"; alert("Hata oluştu.");
    }
}
...
function showPage(p) { if(p==='home'){ hideAllSections(); document.getElementById('welcome-box').classList.remove('hidden'); document.getElementById('stats-card').classList.remove('hidden'); document.getElementById('searchInput').value=''; } }
...
function showResult(word) {
    ...
    div.innerHTML = `<div class="bg-subtle-light dark:bg-subtle-dark rounded-xl p-6 border border-subtle-light dark:border-subtle-dark mt-8 shadow-md">
        ...
        <div class="space-y-4">
            ...
            ${word.Örnek ? `<div><h3 class="font-bold text-primary">Örnek</h3><p class="border-l-4 border-primary/20 pl-4">${c(word.Örnek)}</p></div>` : ''}
        </div>
    </div>`;
...
function renderAlphabet() {
    document.getElementById('alphabet-list').innerHTML = customAlphabet.filter(h => h !== " ").map(h => `<button onclick="showLetterResults('${h}', 0)" class="w-10 h-10 font-bold rounded bg-subtle-light/50 dark:bg-subtle-dark hover:bg-primary hover:text-white transition-all">${h}</button>`).join('');
}

function showLetterResults(h, p) {
...
function calculateStats() {
    const v = allWords.filter(r => r.Sözcük).length;
    document.getElementById('stats-sentence').innerHTML = `Sözlükte <span class="text-primary font-bold">${v}</span> madde bulunmaktadır.`;
}
...
function convertToGreek(s) { return s ? s.split('').map(c => latinToGreekMap[c] || c).join('') : ""; }
